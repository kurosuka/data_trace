...
<template>
  <div class="multi-checkbox">
    <el-tooltip :content="checkBoxDataLabel.toString()" placement="top" :disabled="checkBoxDataLabel.length == 0">
      <el-select v-model="checkBoxDataLabel" placeholder="请选择" multiple :clearable="false" :collapse-tags="true"
        ...
        <div class="operation" v-if="this.sourceData.length != 0">
          <el-checkbox label="" :indeterminate="isIndeterminate" v-model="checkAll" style="margin-right: 5px"
            @change="checkAllchange" v-if="multiType"></el-checkbox>
        ...
        <el-checkbox-group v-model="checkBoxData" style="max-width: 600px" v-if="multiType" @change="checkChange">
          <div class="check-box-contain" v-for="item in data" :key="item.id">
            ...
            <div class="">
              <el-checkbox style="margin-top: 10px;"
                v-show="_isVisible(filterText, ite.title) && _isParentVisible(filterText, item)" :label="ite.id"
                v-for="ite in item.children" :key="ite.id">{{ite.title}}</el-checkbox>
            </div>
            <el-divider direction="horizontal"></el-divider>
          </div>
        </el-checkbox-group>
        <el-radio-group v-model="checkBoxData" style="max-width: 600px" v-if="!multiType" @change="checkChange">
          <div class="check-box-contain" v-for="item in data" :key="item.id">
            <p v-if="_isParentVisible(filterText, item)">{{item.title}}</p>
            <div class="">
              <el-radio style="margin-top: 10px;"
                v-show="_isVisible(filterText, ite.title) && _isParentVisible(filterText, item)" :label="ite.id"
                v-for="ite in item.children" :key="ite.id">{{ite.title}}</el-radio>
            </div>
            <el-divider direction="horizontal"></el-divider>
          </div>
        </el-radio-group>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        checkBoxData: this.defaultSelect.toString(),
